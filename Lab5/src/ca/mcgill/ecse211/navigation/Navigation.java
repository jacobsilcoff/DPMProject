...
public class Navigation extends Thread {
  ...
  public void turnTo(double theta, int speed) {
    ...
    Lab5.RIGHT_MOTOR.setSpeed(speed);

    // turn using MINIMUM angle
    if (ang < 180) {
      Lab5.LCD.drawString("Ang: " + ang + "deg  ", 0, 5);
      // increase angle
      Lab5.LEFT_MOTOR.rotate(convertAngle(ang), true);
      Lab5.RIGHT_MOTOR.rotate(-convertAngle(ang), false);
    } else {
      ang = 360 - ang;
      Lab5.LCD.drawString("Ang: " + ang + "deg   ", 0, 5); // display angle of rotation
  ...
  public void run() {
    ...
    while (on) {
      ...
      switch (state) {
        case INIT:
          Lab5.LCD.drawString("State: INIT", 0, 6);
          if (isNavigating) {
            state = State.TURNING;
          }
          if (correction != null) {
        ...
        case TURNING:
          ...
          break;
        case TRAVELING:
          Lab5.LCD.drawString("State: TRVL", 0, 6);
          updateT();
  ...
  private static int convertDistance(double distance) {
    return (int) ((180.0 * distance) / (Math.PI * Lab5.WHEEL_RAD));
  }
  ...
  private static int convertAngle(double angle) {
    return convertDistance(Math.PI * Lab5.TRACK * angle / 360.0);
  }
