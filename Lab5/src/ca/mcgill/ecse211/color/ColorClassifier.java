...
/**
 * This is a routine that can be used to identify the color of a 
 * soda can located in front of the robot, assuming that the can is 
 * centered under the axis of rotation of the light sensor arm
 * @author jacob
 */
public class ColorClassifier extends Thread{
  ...
  /**
   * Starts by checking if the arm has been calibrated to zero.
   * if it has, 
   */
  public void run() {
    if (!calibrated) {
  ...
  /**
   * Ensures that the tachometer is zeroed properly by
   * moving the arm as far CCW as possible and then back to
   * the rest position.
   */
